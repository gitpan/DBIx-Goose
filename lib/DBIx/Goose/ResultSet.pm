package DBIx::Goose::ResultSet;

=head1 NAME

DBIx::Goose::ResultSet - Methods for searching and altering tables
...
sub find {
    my ($self, $fields, $c) = @_;
    if (scalar @$fields == 0) { push @$fields, '*'; }
    my ($stmt, @bind) = $sql->select(-columns => $fields, -from => $self->{table}, -where => $c);
    my ($wstmt, @wbind) = $sql->where($c);
...
sub insert {
    ...
    if ($res->count) {
        ...
        return bless $rs, 'DBIx::Goose::Result';
    }
    else { return 0; }    
}
